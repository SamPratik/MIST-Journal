...
<body>
	<?php include_once("sectionEditorNavigationBar.php"); ?>
        ...
        <?php  
			$sql = "SELECT rFn,rLn,seFn,seLn,se_message,r_message,se_article,r_article FROM se_reviewer_meassage AS m, section_editor AS s, reviewer AS r WHERE se_id={$seId} AND r_id={$rId} AND r.id=m.r_id AND m.se_id=s.id";
			$result = mysqli_query($connection,$sql);
		?>
        
        <div class="editor-copy-editor-message-container">
        
          <h1>Full conversession with Reviewers</h1>  
          ...
          <?php if($row["r_message"] != NULL) { ?>
          <div class="editor-comment w3-card-4">
              <strong style="display:block;"><?php echo $row["rFn"]." ".$row["rLn"]; ?>(Reviewer)</strong>
              <small style="display:block;">SEPTEMBER 17, 2013</small>
              <p><?php echo nl2br($row["r_message"]); ?></p>
          </div>
